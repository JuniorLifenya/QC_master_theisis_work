...
labels = [
    "EFEs\nlinearised\n$g_{\\mu\\nu}=\\eta_{\\mu\\nu}+\\kappa h_{\\mu\\nu}$",
    "Tetrads\n$e^a_{\\mu}=\\delta^a_{\\mu}$\n$+\\frac{\\kappa}{2}h^a_{\\mu}$",
    "Spin conn.\n$\\omega^{ab}_{\\mu}=$\n$\\frac{\\kappa}{2}(\\partial^a h^b_{\\mu}-\\cdots)$",
    "Belinfante\n$\\mathcal{L}_{\\rm int}=$\n$-\\frac{\\kappa}{2}h_{\\mu\\nu}T^{\\mu\\nu}_{\\rm BR}$",
...
chapters = {0.5:'Ch.2', 4.1:'Ch.3', 7.7:'Ch.4', 11.3:'Ch.5'}

for i,(xc,txt,col) in enumerate(zip(xs, labels, colors)):
    p = FancyBboxPatch((xc-0.82, 0.5), 1.64, 2.9,
                       boxstyle="round,pad=0.1",
                       facecolor=col, edgecolor='white', lw=1.2, alpha=0.9)
    ax.add_patch(p)
...
ax.set_title('Logical derivation chain of the thesis', fontsize=12, color='white')
fig.patch.set_facecolor('#0a0a14')
ax.set_facecolor('#0a0a14')
plt.tight_layout()
plt.savefig('figures/fig_derivation_chain.png', dpi=200, bbox_inches='tight', facecolor='#0a0a14')
