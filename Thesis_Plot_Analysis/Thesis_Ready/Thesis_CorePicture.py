"""
THESIS CENTREPIECE — two-panel storyboard (classical-electron version)
======================================================================
LEFT : GW sheet (Gaussian wells, tanh envelope, turbo radial colormap,
       LightSource hill-shading) with the electron riding the wave.
RIGHT: zoom on the electron drawn in the CLASSICAL picture —
         * intrinsic spin  S : axis arrow + curling arrow,
         * momentum        p : forward arrow,
         * magnetic moment B : dipole field-line loops,
       with the GW arriving from the left and a quantum sensor reading it out.
"""
import numpy as np
...
axL.quiver(ex, ey, ez, 0, 0, 1.0, color="#e6550d", lw=2.4,
           arrow_length_ratio=0.32, zorder=13)
axL.text(ex+1.2, ey-0.88, ez + 1.15, r"$e^-$  (spin-½)",
...
axR.text(0, 0, -0.55, r"$e^-$", fontsize=13, fontweight="bold", ha="center",
...
axR.quiver(0, 0, 0, 1.55, 0, 0, color="#1b9e77", lw=3.4,
           arrow_length_ratio=0.16, zorder=24)
axR.text(1.7, 0, 0.14, r"$\vec{p}$", fontsize=14, fontweight="bold",
         color="#1b9e77", zorder=26)

# --- incoming GW wavefronts from the left ---
