...
ax1.set_title("Single electron", fontsize=12, fontweight="bold", pad=0)
...
for ang in np.linspace(0, np.pi*3, 1, endpoint=False):
    for scl in (0.9, 1.3, 1.7):
        ...
        ax1.plot(bx, by, bz, color="#7e57c2", lw=1.0, alpha=0.40, zorder=3)
...
# Atomic lattice (simplified: fewer dots, no bond lines)
np.random.seed(11)
nx, ny = 22, 4
...
ax2.scatter(XA.ravel(), YA.ravel(), s=16, color="#1f4e96", alpha=0.85,
            edgecolor="none", zorder=5)

# Temperature label
ax2.text(0, 1.55, r"$T \approx 1\,$mK", fontsize=13, ha="center",
         color="#0d6da6", fontweight="bold",
         bbox=dict(boxstyle="round,pad=0.30", fc="#e6f4ff",
                   ec="#0d6da6", lw=1.2))
...
ax2.text(0, -1.70,
         ...
         fontsize=9.5, ha="center", color="0.30", style="italic")
...
ax3.plot(zc, mode_y, color="#c14a14", lw=2.0, zorder=8)
ax3.text(0, 0.32, r"$u(z) = \xi\sin(\pi z/L)$",
         fontsize=9.5, ha="center", color="#c14a14",
...
for offset, alpha in [(0.0, 0.85), (0.45, 0.55), (0.9, 0.30)]:
    ...
    ax3.plot(xw, yw, color="#0E796D", lw=1.5, alpha=alpha)
ax3.text(3.2, 1.40, r"$h(t)$", color="#0E796D", fontsize=11,
...
ladder_y0 = -1.18
ladder_y1 = -0.58
ax3.hlines(ladder_y0, ladder_x-0.25, ladder_x+0.35, color="#7c3aed", lw=2.5)
ax3.hlines(ladder_y1, ladder_x-0.25, ladder_x+0.35, color="#7c3aed", lw=2.5)
ax3.text(ladder_x+0.38, ladder_y0, r"$|n=0\rangle$",
         color="#7c3aed", fontsize=10, va="center", fontweight="bold")
...
ax3.text(ladder_x-0.32, (ladder_y0 + ladder_y1)/2, r"$\hat b^\dagger$",
         color="#7c3aed", fontsize=12, va="center", ha="right", fontweight="bold")

ax3.text(0, -1.55,
         r"single-phonon emission:  $|0\rangle \to |1\rangle\;$"
         r"detected as quantum jump",
         fontsize=10, ha="center", color="#7c3aed", fontweight="bold")
...
fig.patches.append(FancyArrowPatch(
    ...
    color="#444", lw=2.4, mutation_scale=24))
fig.text(0.313, 0.66, "embed in", fontsize=10.5, ha="center", color="0.30",
         style="italic", fontweight="bold")
fig.text(0.313, 0.555, "lattice", fontsize=10.5, ha="center", color="0.30",
         style="italic", fontweight="bold")
...
fig.patches.append(FancyArrowPatch(
    ...
    transform=fig.transFigure, arrowstyle="->",
    color="#444", lw=2.4, mutation_scale=24))
fig.text(0.671, 0.66, "drive with", fontsize=10.5, ha="center", color="0.30",
         style="italic", fontweight="bold")
fig.text(0.671, 0.555, r"$\ddot h(t)$", fontsize=11.5, ha="center",
         color="0.30", fontweight="bold")
...
ax_bottom.text(0.50, 0.62,
    ...
    r"\;\;\Longrightarrow\;\;"
    r"\hat H_{\rm int} = \dfrac{L}{\pi^2}\sqrt{\dfrac{M\hbar}{\omega}}\,\ddot h(t)\,(\hat b + \hat b^\dagger)$",
...
ax_bottom.text(0.345, 0.82, "single particle (thesis)", fontsize=8.5, ha="center",
               va="center", transform=ax_bottom.transAxes, color="0.40",
               style="italic")
ax_bottom.text(0.500, 0.45, "collective coordinate", fontsize=8.5, ha="center",
               va="center", transform=ax_bottom.transAxes, color="0.40",
               style="italic")
...
ax_bottom.add_patch(FancyBboxPatch(
    ...
    transform=ax_bottom.transAxes,
    facecolor="none", edgecolor="#c14a14", lw=1.8, zorder=10))
