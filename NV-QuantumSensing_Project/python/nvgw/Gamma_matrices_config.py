...
class DiracMatrices:
        ...
        def setup_alpha_beta_matrices(self):
            ...
            self.alpha_matrices_i = [self.alpha1, self.alpha2, self.alpha3] # Here we have the total αᵢ matrix

        def get_spin_connection_operator(self, omega_mu, derivative_terms): # We need to define the omega_mu
            """
            Construct spin connection operator for gravitational coupling
            Based on your PDF: Γ_μ = (ik/4) ω_μ^{ab} σ_{ab} = (ik/4) h^b_μ;a
            """
